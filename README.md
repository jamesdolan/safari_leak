...
Debug build of `vulkan-sdk-1.4.321.0` which includes alignment issue in `GUARD_BLOCKS`:
...
The particular issue in GLSlang is only triggered in Debug builds due to the `GUARD_BLOCKS` feature that pads allocations in debug mode. This feature also causes `SAFE_HEAP` to throw faults.
...
Start the local server with cross-origin tags (required for SharedArrayBuffer headers):
